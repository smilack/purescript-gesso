module Gesso.Example.PaintApp.Grid
  ( CanvasIO
  , CanvasIO'
  , Pixel(..)
  , Slot
...
component action = HH.slot GC._gessoCanvas unit GC.component appSpec action
...
appSpec :: GApp.AppSpec CanvasState CanvasIO CanvasIO
appSpec =
  { name: "canvas"
...
extractOutput
  :: GTime.Delta
  -> GGeo.Scalers
  -> GSt.Compare CanvasState
  -> Effect (Maybe CanvasIO)
...
clearHighlight =
  GInt.onMouseOut (\_ _ _ s -> pure $ Just s { mouseCell = Nothing })
...
renderApp
  ...
  -> GTime.Delta
  -> GGeo.Scalers
  -> GSt.States CanvasState
  -> Effect Unit
renderApp context _ { canvas } { current } = do
  let { mouseCell, showGrid } = current
  clearBackground
  drawOutline
  ...
  drawCursor { x, y } = drawPixel $ Pixel { x, y, color: current.color }

  drawImage :: Effect Unit
  drawImage = sequence_ $ map drawPixel $ reverse current.pixels
